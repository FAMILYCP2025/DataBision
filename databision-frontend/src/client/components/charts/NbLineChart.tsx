import { useEffect, useRef } from 'react'
import * as echarts from 'echarts'
import type { ChartDataPoint } from './NbBarChart'
import { CHART_COLORS } from './chartTheme'

interface NbLineSeries {
  name: string
  data: ChartDataPoint[]
  color?: string
}

export interface NbLineChartProps {
  series: NbLineSeries[]
  loading?: boolean
  height?: number
  valueFormatter?: (v: number) => string
  smooth?: boolean
  showArea?: boolean
}

export function NbLineChart({
  series,
  loading = false,
  height = 260,
  valueFormatter,
  smooth = true,
  showArea = false,
}: NbLineChartProps) {
  const ref = useRef<HTMLDivElement>(null)
  const chartRef = useRef<echarts.ECharts | null>(null)

  const hasData = series.length > 0 && series.some((s) => s.data.length > 0)

  // Effect 1: init once
  useEffect(() => {
    if (!ref.current) return
    chartRef.current = echarts.init(ref.current)
    const observer = new ResizeObserver(() => chartRef.current?.resize())
    observer.observe(ref.current)
    return () => {
      observer.disconnect()
      chartRef.current?.dispose()
      chartRef.current = null
    }
  }, [])

  // Effect 2: update option when data changes
  useEffect(() => {
    if (!chartRef.current) return
    if (!hasData) {
      chartRef.current.clear()
      return
    }

    const xCategories = series[0]?.data.map((d) => d.name) ?? []
    const showLegend = series.length > 1

    const option: echarts.EChartsOption = {
      grid: {
        left: 60,
        right: 20,
        top: showLegend ? 40 : 12,
        bottom: 60,
      },
      legend: showLegend
        ? {
            top: 0,
            textStyle: { fontSize: 12, color: '#64748B', fontFamily: 'inherit' },
          }
        : undefined,
      tooltip: {
        trigger: 'axis',
        backgroundColor: '#0F172A',
        borderColor: 'transparent',
        borderWidth: 0,
        textStyle: { color: '#fff', fontSize: 12, fontFamily: 'inherit' },
        formatter: (params: echarts.DefaultLabelFormatterCallbackParams | echarts.DefaultLabelFormatterCallbackParams[]) => {
          const arr = Array.isArray(params) ? params : [params]
          const header = arr[0]?.name ?? ''
          const lines = arr.map((p) => {
            const val = typeof p.value === 'number' ? p.value : 0
            const formatted = valueFormatter ? valueFormatter(val) : String(val)
            return `${p.seriesName ?? ''}: ${formatted}`
          })
          return [header, ...lines].join('<br/>')
        },
      },
      xAxis: {
        type: 'category',
        data: xCategories,
        axisLabel: { fontSize: 12, color: '#64748B', fontFamily: 'inherit' },
        axisLine: { lineStyle: { color: '#E2E8F0' } },
        axisTick: { show: false },
      },
      yAxis: {
        type: 'value',
        axisLabel: {
          fontSize: 12,
          color: '#64748B',
          fontFamily: 'inherit',
          formatter: valueFormatter ?? undefined,
        },
        splitLine: { lineStyle: { color: '#E2E8F0' } },
        axisLine: { show: false },
        axisTick: { show: false },
      },
      series: series.map((s, i) => ({
        name: s.name,
        type: 'line' as const,
        data: s.data.map((d) => d.value),
        smooth,
        itemStyle: { color: s.color ?? CHART_COLORS[i % CHART_COLORS.length] },
        lineStyle: { color: s.color ?? CHART_COLORS[i % CHART_COLORS.length], width: 2 },
        areaStyle: showArea
          ? { color: s.color ?? CHART_COLORS[i % CHART_COLORS.length], opacity: 0.12 }
          : undefined,
        symbol: 'circle',
        symbolSize: 4,
      })),
    }

    chartRef.current.setOption(option, { notMerge: true })
  }, [series, valueFormatter, smooth, showArea, hasData])

  if (loading) {
    return (
      <div
        style={{
          height,
          background: '#F1F5F9',
          borderRadius: 8,
          animation: 'pulse 1.5s ease-in-out infinite',
        }}
      />
    )
  }

  if (!hasData) {
    return (
      <div
        style={{
          height,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          color: '#94A3B8',
          fontSize: 13,
        }}
      >
        Sin datos disponibles
      </div>
    )
  }

  return <div ref={ref} style={{ height, width: '100%' }} />
}
