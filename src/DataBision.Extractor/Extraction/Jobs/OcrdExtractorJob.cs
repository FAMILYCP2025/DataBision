...
public sealed class OcrdExtractorJob : IExtractorJob
{
    ...
    private async Task<string?> ReadIncrementalFilter(CancellationToken ct)
    {
        ...
        if (filter is not null)
            _log.LogInformation("OCRD: incremental filter — UpdateDate ge '{From}'", effectiveFrom!.Value.ToString("yyyy-MM-dd"));
        else
            _log.LogInformation("OCRD: no checkpoint — full extraction (pageSize={Top})", _options.PageSize);
    ...
    private async Task<(JsonArray allRows, string usedSelect)> PaginateWithFallback(string? filter, CancellationToken ct)
    {
        ...
        var result = await _paginator.PaginateAsync(SapObject, "BusinessPartners", baseQueryFull,
        ...
        {
            _log.LogWarning("OCRD: full $select failed — retrying with minimal. Error: {Err}", result.LastError);
            var baseQueryMin = $"$select={MinimalSelect}{filterPart}&$orderby=UpdateDate asc";
            var minResult = await _paginator.PaginateAsync(SapObject, "BusinessPartners", baseQueryMin,
                _options.PageSize, _options.MaxPages, ct);
