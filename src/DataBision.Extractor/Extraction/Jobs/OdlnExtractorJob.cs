...
public sealed class OdlnExtractorJob : IExtractorJob
{
    public string SapObject => "ODLN";

    private const string Endpoint   = "api/ingest/sap-b1/deliveries";
    ...
    private readonly ILogger<OdlnExtractorJob> _log;
    private readonly ServiceLayerPaginator   _paginator;

    public OdlnExtractorJob(
        IServiceLayerClient sl, IDataBisionIngestClient ingest,
        ExtractorOptions options, ILogger<OdlnExtractorJob> log,
    ...
    public async Task<ExtractionResult> RunAsync(bool dryRun, bool send, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var result = await _paginator.PaginateAsync(
                SapObject, "DeliveryNotes",
                $"$select={FullSelect}{filterPart}&$orderby=UpdateDate asc",
                _options.PageSize, _options.MaxPages, ct);
            sw.Stop();
            _log.LogInformation("ODLN: {Count} rows in {Ms}ms", result.AllRows.Count, sw.ElapsedMilliseconds);
            if (!send)
        ...
        {
            sw.Stop();
            _log.LogError("ODLN: extraction failed — {Message}", ex.Message);
    ...
    private async Task<ExtractionResult> SendAsync(JsonArray allRows, int pagesFetched, bool hitMaxPages, TimeSpan extractDuration, CancellationToken ct)
    {
        ...
        var mapped = allRows.Where(r => r is not null).Select(r => SapToIngestMapper.MapOdlnRow(r!, ctx)).ToList();
        var batch = new IngestBatch<SapOdlnRow> { TenantId = _options.TenantId, CompanyId = _options.CompanyId, SapObject = SapObject, ExtractionRunId = runId, BatchId = batchId, IngestionMode = _options.Mode, Rows = mapped };
        var resp = await _ingest.SendAsync(Endpoint, batch, ct);
        sw.Stop();
        if (resp.Success) _log.LogInformation("ODLN sent: inserted={Ins}, updated={Upd}, skipped={Skp}", resp.RowsInserted, resp.RowsUpdated, resp.RowsSkipped);
        else _log.LogError("ODLN send failed (HTTP {Code}): {Error}", resp.StatusCode, resp.Error);
