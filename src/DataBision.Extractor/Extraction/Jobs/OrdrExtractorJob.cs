...
public sealed class OrdrExtractorJob : IExtractorJob
{
    public string SapObject => "ORDR";

    private const string Endpoint   = "api/ingest/sap-b1/sales-orders";
    ...
    private readonly ILogger<OrdrExtractorJob> _log;
    private readonly ServiceLayerPaginator   _paginator;

    public OrdrExtractorJob(
        IServiceLayerClient sl, IDataBisionIngestClient ingest,
        ExtractorOptions options, ILogger<OrdrExtractorJob> log,
    ...
    public async Task<ExtractionResult> RunAsync(bool dryRun, bool send, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            sw.Stop();
            _log.LogInformation("ORDR: {Count} rows in {Ms}ms", result.AllRows.Count, sw.ElapsedMilliseconds);
            if (!send)
        ...
        {
            sw.Stop();
            _log.LogError("ORDR: extraction failed — {Message}", ex.Message);
    ...
    private async Task<ExtractionResult> SendAsync(JsonArray allRows, int pagesFetched, bool hitMaxPages, TimeSpan extractDuration, CancellationToken ct)
    {
        ...
        var mapped = allRows.Where(r => r is not null).Select(r => SapToIngestMapper.MapOrdrRow(r!, ctx)).ToList();
        var batch = new IngestBatch<SapOrdrRow> { TenantId = _options.TenantId, CompanyId = _options.CompanyId, SapObject = SapObject, ExtractionRunId = runId, BatchId = batchId, IngestionMode = _options.Mode, Rows = mapped };
        var resp = await _ingest.SendAsync(Endpoint, batch, ct);
        sw.Stop();
        if (resp.Success) _log.LogInformation("ORDR sent: inserted={Ins}, updated={Upd}, skipped={Skp}", resp.RowsInserted, resp.RowsUpdated, resp.RowsSkipped);
        else _log.LogError("ORDR send failed (HTTP {Code}): {Error}", resp.StatusCode, resp.Error);
