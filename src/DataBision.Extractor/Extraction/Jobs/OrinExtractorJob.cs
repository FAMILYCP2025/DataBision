...
/// Same structure as OinvExtractorJob but for credit notes.
/// </summary>
public sealed class OrinExtractorJob : IExtractorJob
{
    ...
    private async Task<(JsonArray rows, string usedSelect)> FetchWithFallback(CancellationToken ct)
    {
        ...
        try
        {
            var q = $"$top={top}&$select={FullSelect}&$orderby=UpdateDate asc";
                                                 ...
                                                 || ex.Message.Contains("400"))
        {
            ...
            var q = $"$top={top}&$select={MinimalSelect}&$orderby=UpdateDate asc";
    ...
    private void LogSample(JsonArray rows)
    {
        foreach (var row in rows.Take(3))
        {
            ...
            _log.LogInformation("  ORIN sample: DocEntry={E}, DocNum={N}, CardCode={C}, DocTotal={T}, UpdateDate={U}",
                entry, num, card, total, upd);
        }
    }
