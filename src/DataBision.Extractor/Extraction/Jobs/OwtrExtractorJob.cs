...
public sealed class OwtrExtractorJob : IExtractorJob
{
    public string SapObject => "OWTR";

    private const string Endpoint   = "api/ingest/sap-b1/stock-transfers";
    private const string FullSelect = "DocEntry,DocNum,DocDate,FromWarehouse,ToWarehouse,UpdateDate";
    ...
    private readonly ILogger<OwtrExtractorJob> _log;
    private readonly ServiceLayerPaginator   _paginator;

    public OwtrExtractorJob(
        IServiceLayerClient sl, IDataBisionIngestClient ingest,
        ExtractorOptions options, ILogger<OwtrExtractorJob> log,
    ...
    public async Task<ExtractionResult> RunAsync(bool dryRun, bool send, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var result = await _paginator.PaginateAsync(
                SapObject, "StockTransfers",
                $"$select={FullSelect}{filterPart}&$orderby=UpdateDate asc",
                _options.PageSize, _options.MaxPages, ct);
            sw.Stop();
            _log.LogInformation("OWTR: {Count} rows in {Ms}ms", result.AllRows.Count, sw.ElapsedMilliseconds);
            if (!send)
        ...
        {
            sw.Stop();
            _log.LogError("OWTR: extraction failed — {Message}", ex.Message);
    ...
    private async Task<ExtractionResult> SendAsync(JsonArray allRows, int pagesFetched, bool hitMaxPages, TimeSpan extractDuration, CancellationToken ct)
    {
        ...
        var mapped = allRows.Where(r => r is not null).Select(r => SapToIngestMapper.MapOwtrRow(r!, ctx)).ToList();
        var batch = new IngestBatch<SapOwtrRow> { TenantId = _options.TenantId, CompanyId = _options.CompanyId, SapObject = SapObject, ExtractionRunId = runId, BatchId = batchId, IngestionMode = _options.Mode, Rows = mapped };
        var resp = await _ingest.SendAsync(Endpoint, batch, ct);
        sw.Stop();
        if (resp.Success) _log.LogInformation("OWTR sent: inserted={Ins}, updated={Upd}, skipped={Skp}", resp.RowsInserted, resp.RowsUpdated, resp.RowsSkipped);
        else _log.LogError("OWTR send failed (HTTP {Code}): {Error}", resp.StatusCode, resp.Error);
