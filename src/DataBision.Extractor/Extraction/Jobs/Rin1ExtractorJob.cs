...
/// <summary>
/// Extracts Credit Memo lines (RIN1) from SAP B1 Service Layer via CreditNotes/$expand=DocumentLines.
/// Falls back to inline embedded check if $expand is rejected.
/// IMPORTANT: RIN1 lines require ORIN header to be in raw.sap_orin first — send ORIN before RIN1.
/// </summary>
public sealed class Rin1ExtractorJob : IExtractorJob
{
    public string SapObject => "RIN1";

    private const string Endpoint = "api/ingest/sap-b1/credit-memo-lines";
    ...
    private readonly ILogger<Rin1ExtractorJob> _log;

    public Rin1ExtractorJob(
        IServiceLayerClient sl, IDataBisionIngestClient ingest,
        ExtractorOptions options, ILogger<Rin1ExtractorJob> log)
    {
        _sl      = sl;
        _ingest  = ingest;
        _options = options;
    ...
    public async Task<ExtractionResult> RunAsync(bool dryRun, bool send, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            _log.LogInformation("RIN1: {Docs} docs, {Lines} lines total in {Ms}ms (strategy={Strat})",
            ...
            if (linesTotal == 0)
            {
                _log.LogWarning("RIN1: no lines found — nothing to send.");
        ...
        {
            sw.Stop();
            _log.LogError("RIN1: extraction failed — {Message}", ex.Message);
    ...
    private async Task<(List<JsonNode> docs, int linesTotal, string strategy)> FetchDocsWithLines(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var rows = await _sl.GetAsync("CreditNotes", q, ct);
        ...
        {
            _log.LogWarning("RIN1: $expand not supported — trying full-document (no $select). Error: {Msg}", ex.Message);
        }

        // Strategy 2: full document without $select — DocumentLines embedded in complete response
        {
    ...
    private int CountAndLogLines(List<JsonNode> docs, string strategy)
    {
        ...
        foreach (var doc in docs)
        {
            ...
            if (!firstLineDumped)
            {
                ...
                if (firstLine is JsonObject jo)
                {
                    ...
                    _log.LogInformation("RIN1 DocumentLines field names ({Strat}): {Fields}", strategy, string.Join(", ", keys.Take(30)));
                }
        ...
        if (totalLines == 0)
            _log.LogWarning("RIN1: DocumentLines absent or empty in all {Count} docs (strategy={Strat})", docs.Count, strategy);
    ...
    private void LogLineSample(JsonArray lines, string docEntry)
    {
        foreach (var line in lines.Take(2))
        {
            ...
            _log.LogInformation("  RIN1 sample: DocEntry={D}, Line={L}, ItemCode={I}, Desc={Desc}, Qty={Q}, Total={T}",
        ...
        List<JsonNode> docs, int totalLines, TimeSpan extractDuration, CancellationToken ct)
    {
        ...
        var mapped = new List<SapRin1Row>(totalLines);
        foreach (var doc in docs)
        {
            ...
            foreach (var line in linesArr.Where(l => l is not null))
                mapped.Add(SapToIngestMapper.MapRin1Row(docEntry, line!, ctx));
        }

        if (mapped.Count == 0)
        {
            _log.LogWarning("RIN1: mapper produced 0 rows — skipping send.");
            return new ExtractionResult { SapObject = SapObject, Success = true, RowsExtracted = 0, Duration = extractDuration };
        }

        var batch = new IngestBatch<SapRin1Row>
        {
        ...
        if (resp.Success)
            _log.LogInformation("RIN1 sent: inserted={Ins}, updated={Upd}, skipped={Skp} in {Ms}ms",
                resp.RowsInserted, resp.RowsUpdated, resp.RowsSkipped, sw.ElapsedMilliseconds);
        else
            _log.LogError("RIN1 send failed (HTTP {Code}): {Error}", resp.StatusCode, resp.Error);
