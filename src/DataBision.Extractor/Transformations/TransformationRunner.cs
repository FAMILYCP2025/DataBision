...
/// <summary>
/// Executes stg.refresh_all(company_id) against the staging PostgreSQL database.
...
{
    public async Task<IReadOnlyList<(string Object, int RowsAffected)>> RefreshAllAsync(
        string companyId, CancellationToken ct = default)
    {
        ...
        await using var cmd = new NpgsqlCommand(
            "SELECT object_name, rows_affected FROM stg.refresh_all(@company_id);", conn);
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            logger.LogInformation("  stg.{Object}: {Rows} row(s) affected", obj, rows);
        }
