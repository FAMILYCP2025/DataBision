...
{
    /// <summary>
    /// Corrects mart process functions that referenced stg.invoice_line (wrong name).
    /// The actual STG table is stg.sales_invoice_line.
    /// Affected: mart.refresh_inventory_process, mart.refresh_sales_process.
    /// </summary>
    public partial class FixMartProcessFunctions : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Fix mart.refresh_sales_process — item_dashboard section used stg.invoice_line
            migrationBuilder.Sql("""
                ...
                BEGIN
                    ...
                    -- FIX: was checking/using stg.invoice_line — correct name is stg.sales_invoice_line
                    IF EXISTS (SELECT FROM information_schema.tables
            ...
            // Fix mart.refresh_inventory_process — used stg.invoice_line in LATERAL joins
            // (unconditionally, not guarded by IF EXISTS — caused 42P01 error when stg.item exists)
            migrationBuilder.Sql("""
                ...
                BEGIN
                    ...
                    -- FIX: LATERAL joins now use stg.sales_invoice_line (was stg.invoice_line)
                    IF EXISTS (SELECT FROM information_schema.tables
        ...
        {
            // Restore original (broken) versions — not useful in practice, but required by EF convention
